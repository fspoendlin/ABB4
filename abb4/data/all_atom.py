...
def atom37_to_atom14(atom37, batch):
...
def compute_backbone(bb_rigids, psi_torsions):
    torsion_angles = torch.tile(
        psi_torsions[..., None, :], tuple([1 for _ in range(len(bb_rigids.shape))]) + (7, 1) # [B, N, 8, 2]
    )
...
def detach_aatypes(aatype, res_mask):
    return aatype.detach().cpu() * res_mask.detach().cpu()
...
def atom37_from_trans_rot(trans, rots, res_mask=None):
    ...
    atom37 = compute_backbone(rigids, torch.zeros(trans.shape[0],
                                                  trans.shape[1],
                                                  2,
                                                  device=trans.device)
                             )[0].detach().cpu()
    
    batch_atom37, num_batch = [], res_mask.shape[0]
    for i in range(num_batch):
        batch_atom37.append(du.adjust_oxygen_pos(atom37[i], res_mask[i].detach().cpu()))
    return torch.stack(batch_atom37)

def atom14_from_trans_rot_torsion(trans, rots, torsions, aatypes):
    rigids = du.create_rigid(rots, trans)

    all_frames_to_global = torsion_angles_to_frames(r=rigids,
                                                    alpha=torsions,
                                                    aatype=aatypes)
    atom14 = frames_to_atom14_pos(all_frames_to_global, aatypes)

    return atom14

def atom37_from_trans_rot_torsion(trans, rots, torsions, aatypes, mapping, mask):
    rigids = du.create_rigid(rots, trans)
    ...
    atom14to37 = {'residx_atom37_to_atom14': mapping,
                  'atom37_atom_exists': mask
                  }
    atom37 = atom14_to_atom37(atom14, atom14to37)

    return atom37.detach().cpu()

def convert_to_coords_and_detach(state_traj, res_mask, torsions=False, batch=None):
    atom37_traj, seq_traj = [], []
    if torsions: # predicted torsion angles
        for trans, rots, aatypes, torsions in state_traj:
            atom37_traj.append(atom37_from_trans_rot_torsion(
                trans,
                rots,
                torsions,
                aatypes,
            ...
            seq_traj.append(detach_aatypes(aatypes, res_mask))
    else: # no predicted torsion angles
        for trans, rots, aatypes in state_traj:
            atom37_traj.append(atom37_from_trans_rot(trans, rots, res_mask))
            seq_traj.append(detach_aatypes(aatypes, res_mask))

    return torch.stack(atom37_traj), torch.stack(seq_traj)
