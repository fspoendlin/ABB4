...
from Bio import PDB
from abb4.data import protein, residue_constants, parsers
...
region_boundaries = [[0, 26],[27, 38], [39, 55], [56, 65], [66, 104], [105, 117], [118, 999]]
imgt2region = np.array((region_boundaries[0][1]-region_boundaries[0][0]+1)*[0] +
             (region_boundaries[1][1]-region_boundaries[1][0]+1)*[1] +
             (region_boundaries[2][1]-region_boundaries[2][0]+1)*[2] +
             (region_boundaries[3][1]-region_boundaries[3][0]+1)*[3] +
             (region_boundaries[4][1]-region_boundaries[4][0]+1)*[4] +
             (region_boundaries[5][1]-region_boundaries[5][0]+1)*[5] +
             (region_boundaries[6][1]-region_boundaries[6][0]+1)*[6])
imgt2region = np.concatenate([imgt2region, imgt2region+7]) # light chain regions offset by 7
...
def load_rigids(processed_file_path, return_feats=False):
    ...
    chain_feats = {
    'aatype': torch.tensor(
        processed_feats['aatype']).long(),
    'all_atom_positions': torch.tensor(
        processed_feats['atom_positions']).double(), 
    'all_atom_mask': torch.tensor(
            processed_feats['atom_mask']).double()
    }
    chain_feats = data_transforms.atom37_to_frames(chain_feats)  # frames
    chain_feats = data_transforms.make_atom14_masks(chain_feats) # atom14 -> atom37 mapping
    chain_feats = data_transforms.atom37_to_torsion_angles()(chain_feats) # torsion angles
    ...
    if return_feats:
        return trans_1, rotmats_1, chain_feats, processed_feats
    else:
