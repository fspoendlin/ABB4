...
def batch_r3_dist_matrix(batch_trans_0, batch_trans_1, mask, num_batch, num_res, return_aligned_noise=False):
    # all combinations of noise and ground truth
    noise_idx, gt_idx = torch.where(
        torch.ones(num_batch, num_batch))
    batch_trans_0 = batch_trans_0[noise_idx]
    batch_trans_1 = batch_trans_1[gt_idx] # sample idx [0, 1, 0, 1]
    batch_mask = mask[gt_idx]

    # align noise to ground truth
    aligned_trans_0, aligned_trans_1, _ = du.batch_align_structures(
        batch_trans_0, batch_trans_1, mask=batch_mask
    )

    aligned_trans_0 = aligned_trans_0.reshape(num_batch, num_batch, num_res, 3)
    aligned_trans_1 = aligned_trans_1.reshape(num_batch, num_batch, num_res, 3)
    batch_mask = batch_mask.reshape(num_batch, num_batch, num_res)

    r3_dist = torch.sum(
        torch.linalg.norm(aligned_trans_0 - aligned_trans_1, dim=-1) * batch_mask, dim=-1
    ) / torch.sum(batch_mask, dim=-1)

    if return_aligned_noise:
        return r3_dist, aligned_trans_0
    else:
        return r3_dist

def batch_so3_dist_matrix(batch_rotmats_0, batch_rotmats_1, mask, num_batch, num_res):
    # all combinations of noise and ground truth
    noise_idx, gt_idx = torch.where(
        torch.ones(num_batch, num_batch))
    batch_rotmats_0 = batch_rotmats_0[noise_idx]
    batch_rotmats_1 = batch_rotmats_1[gt_idx] # sample idx [0, 1, 0, 1]
    batch_mask = mask[gt_idx]

    batch_rotmats_0 = batch_rotmats_0.reshape(-1, 3, 3)
    batch_rotmats_1 = batch_rotmats_1.reshape(-1, 3, 3)
    batch_mask = batch_mask.reshape(num_batch, num_batch, num_res)

    relative_angles = so3_relative_angle(batch_rotmats_1, batch_rotmats_0).reshape(num_batch, num_batch, num_res)
    so3_dist = torch.sum(
        relative_angles * batch_mask, dim=-1
    ) / torch.sum(batch_mask, dim=-1)
